...
// https://github.com/sanger/xultree/blob/master/chrome/content/treedata.js

var EXPORTED_SYMBOLS = ["TreeItem", "nsTreeView"];
...
function nsTreeView(dataArray, getCellTextCB) {
	...
	//this._json = json;
    this._getCellTextCB = getCellTextCB;
	this._treeData = [];
	this._treeCache = {};
	this._level = 0;
	this._count = 0;
...
//		}			
	...
	getCellText: function getCellText(row, col){
		var item = this._get_tree_item_by_row(row);
		return this._getCellTextCB(item, row, col);
	},   
     ...
     * 
     * @param   {TreeItem}     parent     Description
     * @param   {Object}       data       Description
     * @param   {Array}        children   Description
     * 
     * @returns {TreeItem}   Description
     */
    addItem: function addItem(parent, data, children)
    {
          ...
          {
               ti.isContainer = true;
               ti.isContainerEmpty = (data.children.length == 0);
          }
               
          ti.id = this._count;
          ti.level = (parent == null) ? 0 : parent.level + 1;
