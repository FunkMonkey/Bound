...
\myProperName{JavaScript} uses prototypes instead. A prototype object is a normal \myProperName{JavaScript} object, from which other objects inherit properties. When \myProperName{JavaScript} looks up a property of an object, the object is checked if it does have an own property with the name. If not, its prototype is checked for the property. If this doesn't have it either, the prototype's prototype is checked and so on up its hierarchy. This hierarchy is called the prototype chain.\\
...
One way of caching \mySCName{JSObject}s is to store the first created \mySCName{JSObject} in a map along with the memory address of the instance that is wrapped. Whenever the same instance needs to be wrapped, the map is checked for the memory address and if an entry exists, the according \mySCName{JSObject} will be used instead of rewrapping the \myProperName{C++} instance. However, there are several drawbacks with this approach.
