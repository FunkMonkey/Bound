\chapter{Binding C++ and \myProperName{SpiderMonkey}}
\label{chap:LanguageBindingCPPJS}

This chapter shows how \myProperName{C++} elements like declarations and types can be exposed to \myProperName{Mozilla SpiderMonkey} to be used from scripts. \myProperName{SpiderMonkey} is the name of \myProperName{Mozilla}'s implementation of the \myProperName{ECMAScript}\footnote{\myProperName{ECMAScript}: \url{http://www.ecmascript.org/}} programming language standard, which is most commonly known as \myProperName{JavaScript}. The terms \myProperName{JavaScript} and \linebreak \myProperName{ECMAScript} will be used interchangebly, whereas \myProperName{SpiderMonkey} refers to the \linebreak concrete \myProperName{Mozilla} implementation.\\
...
\section{\myProperName{JavaScript}}
...
Boolean, number and string are primitive types. So are \mySCName{null} and \mySCName{undefined}, which both represent the absence of value. All these types are immutable.\\
...
When converting between \mySCName{jsval}s and \myProperName{C++} types, type checking is indispensable. The \myProperName{C++} compiler that compiles the glue code will perform this task, when converting \textbf{to} \mySCName{jsval}, as the conversion functions only accept the correct type. When converting a \mySCName{jsval} to a \myProperName{C++} type, the actual type of the \mySCName{jsval} needs to be checked manually first. If the \myProperName{JSAPI} user does not perform type checking and, for example, retrieves a \mySCName{JSObject} from a \mySCName{jsval} that contains a boolean value, the behaviour of the code is undefined and will probably lead to a run-time failure like a crash. Thus type checking and error handling play an important role and are explained in further detail \linebreak in \myRefSection{sec:ErrorHandling}.\\
\myProperName{SpiderMonkey} also provides utility functions for automatic conversion of types \linebreak according to the \myProperName{ECMAScript} standard. If, for example, a boolean is passed to a function that expects a string, the conversion would create a string from the boolean --- either \mySCString{true} or \mySCString{false}. These conversion functions are less performant, but allow for a greater range of input values without reporting errors. It has to be said though that automatic conversion can lead to unexpected program behaviour in places where the user would expect an error.\\
Most of the conversion functions take a \mySCName{JSContext} as the first parameter.

\section{Boolean Values}

Boolean values can be converted between \mySCName{jsval}s and the \myProperName{C++} boolean type using \mySCName{JSVAL\_TO\_BOOLEAN} and \mySCName{BOOLEAN\_TO\_JSVAL}. \mySCName{JS\_ValueToBoolean} can be used for \linebreak automatic conversion. The latter function is, for instance, able to convert the string \mySCName{jsval} \mySCString{true} to a \myProperName{C++} boolean. If the function returns false, the given \mySCName{jsval} could not be converted.
...
When converting to integers, the number value is rounded according to the \linebreak\myProperName{ECMAScript} standard.
...
\myProperName{JavaScript} strings (\mySCName{JSString}) are sequences of 16 bit wide characters. In \linebreak\myProperName{SpiderMonkey} such a character is defined as a \mySCName{jschar} and \mySCName{JSString} is a sequence of \mySCName{jschar}s. A \mySCName{jschar} is UTF-16.
...
An example for converting strings can be found in \myRefListing{lst:ConvertStrings}.

Because \mySCName{std::string}s can be created from character arrays and a \mySCName{const char*} can be retrieved using the \mySCName{c\_str} function, converting between \mySCName{JSString} and \mySCName{std::string} works similarly.
...
Because of that, \myProperName{C++11} introduces new character types for unicode \linebreak characters: \mySCName{char16\_t} and \mySCName{char32\_t}. Having a fixed width, \mySCName{char16\_t} should be the same as a \mySCName{jschar} on all platforms and therefore be best suited for conversion between unicode strings and \mySCName{JSString}.

\section{Namespaces}

In \myProperName{C++}, namespaces exist to create a scope for entities like functions and \linebreak classes --- effectively grouping them. In \myProperName{JavaScript} the same task can be achieved by creating a hierarchy of \mySCName{JSObject} instances. \mySCName{JS\_DefineObject} can be used to define an object property on another object.\\ \myRefListing{lst:WrappingNamespace} shows how nested namespaces are wrapped.
...
Primitive types, including strings, are immutable. As such, functions that take a reference or pointer to a non-const immutable type (e.g. \mySCName{int\&} or \mySCName{char[]}) and change the value of the parameter during the call (also known as \mySlang{out-parameter}) will not effect the actual \myProperName{JavaScript} variable that was passed. In such cases the wrapper functions must be written differently. Script calls to \myProperName{C++} functions with out-parameters, whose types are immutable in \myProperName{JavaScript}, will look different compared to \myProperName{C++} calls.
...
Problems can arise for functions that overload on \myProperName{C++} types that have the same representation in \myProperName{JavaScript}, for example \mySCName{float} and \mySCName{double}. Given a number value, there is no way to decide on the correct overload implementation in such a case.
...
\myRefListing{lst:InitClass} showed how the constructor stores \myProperName{C++} class instances in the private data \mySCName{void*} of a \mySCName{JSObject}.
...
If these checks succeed, a private data value can safely be converted to the correct \myProperName{C++} type using \mySCName{static\_cast} as shown in \myRefListing{lst:UnwrappingPriv} \textit{(next page)}.
...
The wrapping of member functions works similarly to the wrapping of free functions as explained in \myRefSection{sec:Functions}.
...
\myRefListing{lst:GlueAccessor} \textit{(next page)} shows the glue code for the getter and setter of an accessor property.
...
In contrast to functions and accessor properties, value properties for non-static members are defined on the instance-wrapping \mySCName{JSObject} and not on the prototype. Thus \linebreak\mySCName{JS\_DefineProperty} needs to be called, wherever the corresponding \mySCName{JSObject} is \linebreak created --- for example in the constructor or a function that wraps an existing instance.
...
Static data members and functions are wrapped similarly to fields and functions. As they are static, they make no use of the \mySCName{this} object. When calling \mySCName{JS\_InitClass}, static properties and functions can be passed. \myProperName{JavaScript} has no real concept of static members as there is no distinction between classes and instances. When declaring ``static'' properties using \mySCName{JS\_InitClass}, these are simply defined on the created \linebreak constructor function, so access to static members looks similar to \myProperName{C++}. Contrary to \myProperName{C++}, it is not possible to access a static member from within an instance \linebreak using \mySCName{this} --- static members always have to be accessed using the constructor function object.
...
The \myProperName{JSAPI} user has to decide upon which side is responsible for freeing \linebreak the memory --- meaning which party holds the \textbf{ownership}. Depending on the \linebreak application, this is not necessarily the party that allocated the memory.
...
The glue code for wrapping a single class will generally make use of one of these three abstract patterns, depending on how the user intends to use the class from script. As strict script and strict native ownership will apply to all wrapped objects (hence the term \textbf{strict}), the created \mySCName{JSObject}s do not need to store state of the ownership. The finalizer will be written according to the ownership.\\
...
Dangling pointers occur in several places. Deleted instances may be referenced by the \myProperName{C++} code of the wrapped library or by the private data of \mySCName{JSObject}s (hence, in the glue code). When using a dangling pointer, it makes no difference, which side falsely deletes or uses it.
...
In terms of language binding, the consequences of using dangling pointers are \linebreak\mySlang{acceptable}, when the dangling pointers result from bugs in the application logic (\myProperName{C++} or script). These dangling pointers are acceptable, because there is no way of preventing them in the glue code.
...
Global variables are wrapped similarly to class fields as described in \myRefSection{sec:Fields}. Instead of defining the property on the class prototype or the instances, it is defined on the global \myProperName{JavaScript} object or the \mySCName{JSObject} that represents the \linebreak corresponding namespace.
...
Because of the queue that does not know about execution order, the callback that prints \mySCString{Hello} is called first. The result should be the other way around, because the time delay for \mySCString{Hello} is 10 and thus after \mySCString{World}. 
...
Either way, passing state information demands modification of the original source code. This may prove to be impossible in cases where third party libraries are used.

\subsubsection{Run-time Function Creation and Closures}

\myProperName{C++11} allows the creation of functions \textbf{at run-time} with the help of lambda functions\footnote{Lambda functions: \url{http://www.cprogramming.com/c++11/c++11-lambda-closures.html}}. Lambda functions can capture state and thus seem to be capable of solving the given problem. This concept is also known as a closure and --- as a side note --- is very common in \myProperName{JavaScript}.
...
\myProperName{SpiderMonkey} also provides functions for getting and accessing arbitrary \myProperName{JavaScript} objects and values. Object properties can be inspected and \myProperName{JavaScript} functions can be called.
...
The functionality to access arbitrary \myProperName{JavaScript} objects and calling \myProperName{JavaScript} \linebreak functions from \myProperName{C++} makes it possible to use \myProperName{JavaScript} libraries from native code.