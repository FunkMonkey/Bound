...
\textbf{Dynamic binding, dynamic dispatch} or \textbf{\mySlang{late} binding}, on the other hand, refers to the address of the code to be called being acquired dynamically at run-time. This can be done in various ways, and the definitions found in literature and online vary widely at this point\todo{word}.
...
Note, that in contexts outside of standard \myProperName{C++}, for example in \myProperName{Microsoft COM}, dynamic binding using RTTI is still sometimes referred to as \mySlang{early} binding, due to its performance advantage compared to the lookup using an identifier.
...
\myProperName{SWIG} does not include a full \myProperName{C++} parser by design. The parser in \myProperName{SWIG} was created in a way that it can handle incomplete interfaces and deal with missing type declarations.\footnote{\citep{SWIGHPDoc} in 5.1.6 Parser Limitations}
