...
There are languages that lie in between these definitions, being statically typed as well as interpreted or byte-code compiled, for example \myProperName{C\#} and \myProperName{Java}. 
...
When binding together a system programming language and a scripting language, we need to differentiate between \textbf{extending} and \textbf{embedding} of a scripting language.\footnote{\citep{PythonHP}} Languages like \myProperName{Python} that run inside a virtual machine come with their own interpreter, which often also exists as an independent executable. The developers also provide a library that exposes the scripting language's extending/embedding API. \\
...
The lookup of native functions may be trivial for programming languages that support reflection, like \myProperName{C\#} or \myProperName{Java}, where the language provides ways of calling functions using a string-identifier.\\
...
Note, that in contexts outside of standard \myProperName{C++}, for example in \myProperName{Microsoft COM}, dynamic binding using RTTI is still sometimes referred to as ''early'' binding, due to its performance advantage compared to the lookup using an identifier.
...
Example taken from the \myProperName{Mozilla Developer Network}\footnote{\url{https://developer.mozilla.org/En/SpiderMonkey/JSAPI_User_Guide}}.
...
Calls to an object's functions are resolved using dynamic method invocation based on an identifier as explained in \myRefSection{sec:TypesOfFunctionBinding}. Consider a script object that wraps a class instance. The object internally holds a reference to the wrapped instance. When calling a method of the given object, f. ex. \mySCName{doSomething}, the call will be directed to the proxy function. Based on the given string ''doSomething'' and the internal instance, the proxy will look up the memory address and expected parameters of the native function, convert the given script-arguments to the expected format, call the native function and convert the return value to a type that is understood by the scripting language - all at run-time.
...
Such components need to conform to component standards, which ''are embodied in a component model and define [...] how component interfaces should be specified and how components communicate.''\footnote{\citep[440]{Sommerville}} \myAuthorName{Polberger} further notes that they also standardize facets like memory management, exception- and error-handling as well as metadata for runtime type information.\footnote{\citep[8]{ComponentTechnology}}

Prominent examples of such component models (standards and middleware) are the \myProperName{Object Management Group's (OMG) Common Object Request Broker Architecture (CORBA)} and \myProperName{Microsoft's Component Object Model (COM)}. \myProperName{Mozilla} uses a cross-platform variant of \myProperName{COM} called \myProperName{XPCOM} inside its products.
...
Components written in \myProperName{COM} have to follow a binary standard, meaning they have to share the same layout in memory for a given interface, no matter in which language they are implemented. Not all programming languages support \myProperName{COM}s binary standard, thus there is only a set of languages \myProperName{COM} components can be written in, though a broader set they can be used from.
...
\myProperName{SWIG}, an abbreviation for Simplified Wrapper and Interface Generator, expects input files containing \myProperName{ANSI C/C++} declarations annotated with special \myProperName{SWIG} directives that will later be used to create the glue code. Although the program contains a feature-rich \myProperName{C++} parser that is able to parse most \myProperName{C++} header files, most users will provide special \myProperName{SWIG} interface files containing the declarations to be wrapped. This is useful for filtering and customizing the output, especially in cases where the original header files can not be modified. \myProperName{SWIG} is called through its command-line interface, providing the interface- or header file as an argument.
...
\myProperName{SWIG} does not include a full \myProperName{C++} parser by design. The parser in \myProperName{SWIG} was created in a way that it can handle incomplete interfaces and deal with missing type declarations.\todo{ref}

The development of \myProperName{SWIG} already started in 1995\footnote{\citep{SWIGHPDoc} in 1.1 Introduction}. The application can wrap most \myProperName{C++} constructs to be used in a multitude of scripting languages, while allowing extreme customization of the output code and its behaviour. \myProperName{SWIG}'s detailed documentation\footnote{\url{http://www.swig.org/Doc2.0/Contents.html\#Contents}} contains a lot of background information - not just about the usage of \myProperName{SWIG} itself, but also about the problems that can arise when binding languages, both for language binding tools and their users. It also explains the design decisions behind \myProperName{SWIG} to circumvent these problems.
...
\myLBHighlight{
This thesis does not have the purpose of reinventing the wheel, but to tackle the same problem in a different way.\\
& \\
...
& \\
& Just like \myProperName{SWIG}, the application should be extendable in an easy fashion. As explained in TODO:REF the \myProperName{Mozilla Framework}, which also serves as the foundation for the \myProperName{Firefox} browser, will be used for easy extendability. This will allow language modules to be written in \myProperName{JavaScript} instead of \myProperName{C++}, which is needed in \myProperName{SWIG}. This brings all the benefits (and drawbacks) of using scripting languages to the language binding tool itself.\\
...
& \\
& Although the tool can be used as a replacement for \myProperName{SWIG}, the general idea for future releases is to also provide a \myProperName{SWIG} language module that is able to produce \myProperName{SWIG} interface files - turning the application into a GUI frontend for \myProperName{SWIG} and in this way add support for languages not supported by \myProperName{Bound} itself.\\
& \\
& The first language module for this project is a scripting language currently not supported by \myProperName{SWIG}: the \myProperName{Mozilla} implementation of the \myProperName{ECMAScript} Standard, commonly known as \myProperName{JavaScript}, called \myProperName{Spidermonkey}.TODO:REWRITE\\
& \\
& Compared to \myProperName{SWIG}, \myProperName{Bound} will include a complete \myProperName{C++} parser, based on the \myProperName{Clang} \myProperName{C}-language family compiler. This has the advantage of outsourcing the \myProperName{C++} parsing to an actively developed project that deals with implementing new language features that change syntax, like the auto-keyword or the alternative function syntax coming with the new \myProperName{C++} standard \myProperName{C++11}.
}
