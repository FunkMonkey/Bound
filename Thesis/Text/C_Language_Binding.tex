...
Both \myProperName{Python} and \myProperName{Mozilla SpiderMonkey} (\myProperName{JavaScript}) are accessible in form of a library exposing a \myProperName{C} API. Thus glue code for binding \myProperName{C} or \myProperName{C++} functionality to one of these languages has to be written in \myProperName{C/C++} using the given API.
...
The glue code generated by \myProperName{SWIG} will need no further modification by the user, but can instead be compiled as is.
...
\myLBHighlight{The application should not be restricted to language binding, but allow extensions that use \myProperName{C++} syntax trees in different fashions (documentation, etc.) as described in \myRefSection{sec:AreaOfApplication} of static analysis tools.\\
& \\
