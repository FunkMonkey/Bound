...
Both Python and Spidermonkey (Mozilla's implementation of the ECMAScript standard, also known as JavaScript) are accessible in form of a library exposing a C-API. Thus glue code for binding C or C++ functionality to one of these languages has to be written in C / C++ using the given API.
...
The call of the \underline{native} function \textbf{rand} itself uses static binding or when working with virtual classes (for calling methods) dynamic binding using RTTI. As JavaScript itself is a weakly typed language, the call to the \underline{wrapper} function \textbf{myjs\_rand} itself is in fact dynamically bound and its address is looked up using the string ''rand''.

Looking at the given example, it is obvious that even the wrapping of a simple C function, which does nothing but return a double value without taking any argument, requires an amount of glue code that must not be underestimated and which is most-likely error-prone and hard to maintain.

\myLBHighlight{Thus, the main subject of this thesis is to develop an application that assists in the creation of glue code for binding different scripting languages to C++ using a static foreign interface as presented above for Mozilla Spidermonkey.}
...
\myLBHighlight{Thus SWIG serves the exact same purpose as the application developed in this thesis.}
...
& \\
& SWIG is a command-line utility. For enhanced usability the application created in this thesis will be a GUI application that allows the exploration of a C++ syntax tree, creation of an output tree for multiple languages and customization of export options using GUI elements.\\
& \\
& Just like SWIG, the application should be extendable in an easy fashion. As explained in TODO:REF the Mozilla framework, which also serves as the foundation for the Firefox browser, will be used for easy extendability. This will allow language modules to be written in JavaScript instead of C++, which is needed in SWIG. This brings all the benefits (and drawbacks) of using scripting languages to the language binding tool itself.\\
& The application should not be restricted to language binding, but allow extensions that use C++ syntax trees in different fashions (documentation, etc.) as described in \myRefSection{sec:AreaOfApplication} of static analysis tools.\\
& \\
& Although the tool can be used as a replacement for SWIG, the general idea for future releases is to also provide a SWIG language module that is able to produce SWIG interface files - turning the application into a GUI frontend for SWIG and in this way add support for languages not supported by Bound itself.\\
& \\
& The first language module for this project is a scripting language currently not supported by SWIG: the Mozilla implementation of the ECMAScript Standard, commonly known as JavaScript, called Spidermonkey\\
& \\
& Compared to SWIG, Bound will include a complete C++ parser, based on the Clang C-language family compiler. This has the advantage of outsourcing the C++ parsing to an actively developed project that deals with implementing new language features that change syntax, like the auto-keyword or the alternative function syntax coming with the new C++ standard C++11.
}
