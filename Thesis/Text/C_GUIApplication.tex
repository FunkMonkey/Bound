...
Being based on web technologies, especially \myProperName{JavaScript}, development with  \linebreak\myProperName{XULRunner} is very fast.
...
\mySCName{parse\_header} returns the stringified JSON representation of the \myProperName{C++} AST, as \linebreak described in \myRefSection{sec:ExportJSON}. The string is deserialized into a \myProperName{JavaScript} object using \myProperName{JavaScript}'s \mySCName{JSON.parse} function.
...
As can be seen in \myRefListing{lst:TemplateBool}, the template code contains the placeholders \mySCName{\{\$jsvalName\}} and \mySCName{\{\$inputVar\}}. After creating the \myProperName{jSmart} template by calling the \mySCName{jSmart} \linebreak constructor, the \mySCName{fetch} function can be used to process the template. The function requires an argument that contains the values to be inserted for the \linebreak placeholders (\mySCString{jsBool} and \mySCString{cppBool}).
...
Here is a list of the currently existing restrictions:
...
Prerequisites: A type map entry exist for \mySCName{int}. A type library entry exists for \linebreak\mySCName{WrappedClass}.
...
\mySCName{prepareAndDiagnose} will accumulate all data that is necessary for the generation \linebreak process. Most of the data will be retrieved from the \mySCName{Export\_ASTObject}, the \linebreak \mySCName{sourceObject} and the options set for the entity code generator itself. All data is saved in a property named \mySCName{\_genInput}. While the data is accumulated, it is checked for various sources of errors. If errors or warnings occur, these are stored in \mySCName{\_genInput.diagnosis}.\\
...
Preparation happens in multiple rounds, meaning that the whole tree is traversed \linebreak multiple times and \mySCName{prepareAndDiagnoseRound} called with the current round passed as an \linebreak argument. This is necessary for cross-referencing. A function that needs a type library entry of a class that comes later in the tree would issue wrong errors, just because the corresponding type library has not been created yet or is not up to date. Therefore, type library entries are created in the first round, while the functions just start preparing in the second round, when it is safe to use the type library entries.
...
All used templates (for parameters, return type and the function itself), are  checked, whether they provide include dependencies. All includes are collected, duplicates \linebreak eliminated and the list stored in the result object, which is then returned.
...
As the \myProperName{C++} projects, for which glue code is created, may be under development, new functions or classes are likely to be added or removed. In such a case, the \myProperName{C++} AST has to be reparsed for the loaded project. A new \mySCName{CPP\_AST} is retrieved by calling \myProperName{CPPAnalyzer}. This invalidates all \mySCName{Export\_ASTObject}s as their \mySCName{sourceObject} \linebreak properties still reference the old \mySCName{CPP\_ASTObject}s. Thus, the objects have to be \linebreak reconnected to the new AST nodes by using the USRs of the old \mySCName{sourceObject}s. If the current \mySCName{sourceObject} is \mySCName{null}, the \mySCName{\_sourceObjectSaveObject} property is checked to recall information about the USR. If the \mySCName{sourceObject} is not \mySCName{null}, but the \linebreak corresponding USR is not in the new AST, then the old USR is stored in \linebreak\mySCName{\_sourceObjectSaveObject} and the \mySCName{sourceObject} set to \mySCName{null}. This makes it possible to reconnect to the correct \mySCName{CPP\_ASTObject}, even if it got missing in a set of \linebreak reparse-operations, for example in case the \myProperName{C++} function/class/entity has been \linebreak accidentally deleted in the source code and was readded later.
...
Adding new property row types is done using \mySCName{GUIPropertyFactory}s, which are \linebreak responsible for creating the GUI elements. The \mySCName{ObjectExplorerPropertyManager} \linebreak handles a list of which property factories are connected with which types (based on a string representation of the \myProperName{JavaScript} type). \mySCName{GUIPropertyFactoryString} would be associated with \mySCString{string}, \mySCName{GUIPropertyFactoryColor} with \mySCString{Color}, and so on. 
...
The \mySCName{ObjectExplorer} has no knowledge of the specific \myProperName{JavaScript} object that it \linebreak displays, as this is completely handled by the \mySCName{DataHandler}. This means, that a custom  \mySCName{DataHandler} could combine the properties of multiple objects for showing them as a unit within an \mySCName{ObjectExplorer}.
...
\myProperName{XUL} already comes with a native tree widget, but it requires the user to manage the hierarchy of visible tree elements in the form of a list. The user himself has to track whether nodes have been opened, which child nodes are visible and adjust the list accordingly.
