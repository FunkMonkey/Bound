...
For this reason, entity code generators make use of templates (not to confuse with \myProperName{C++} templates). A template is basically a string of \mySlang{incomplete} source code. It is \mySlang{incomplete} in the sense that it contains placeholders that will be filled with proper code when processing the template with the template engine. The template engine used in this project is \myProperName{jSmart}\footnote{\url{http://code.google.com/p/jsmart/}}, a \myProperName{JavaScript} port of the \myProperName{PHP} template engine \myProperName{Smarty}\footnote{\url{http://www.smarty.net/}}.
...
Types can generally be converted from \myProperName{C++} to the scripting language and verse visa.
