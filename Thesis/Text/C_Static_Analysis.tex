...
There are many more possible sources for bugs that can be detected using static analysis tools, such as division by zero, dereferencing of \mySCName{null}-pointers or the returning of references or pointers to local function stack variables.
...
\\On the one hand, \myProperName{Clang} itself can be included as a shared library. The internal structures and functions of the compiler can be used to access the AST build by \myProperName{Clang} in a direct manner.
