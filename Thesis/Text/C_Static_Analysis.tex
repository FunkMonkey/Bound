...
\myInlineQuote{The lint program for C programs is generally considered the first widely used static analysis tool for defect detection.}\footnote{\citep[1]{UsingSAToFindBugs}}
...
Prominent tools like \myProperName{Doxygen}\footnote{\url{http://www.stack.nl/~dimitri/doxygen/}} automatically create documentation from source code and code comments.
...
Also, as this project is meant to be open-source, its dependencies must be released in an according licence, which excludes all commercial static analysis tools like \myProperName{Coverty}\footnote{\url{http://www.coverity.com/}}, \myProperName{PVS-Studio}\footnote{\url{http://www.viva64.com/en/pvs-studio/}} or \myProperName{pc-lint}\footnote{\url{http://www.gimpel.com/html/pcl.htm}}. It is also not known, whether these tools would provide the ability to export symbol and type information.
...
\myProperName{Dehydra} is developed by \myProperName{Mozilla} for checking specific bug patterns in its large codebase for \myProperName{Firefox} and other \myProperName{Mozilla} applications. It allows custom querying of the AST with the help of \myProperName{JavaScript} scripts.
...
The open-source \myProperName{Clang} project is not only concerned with developing a compiler that produces high-performance code, but also with developing software that can be used in utilities like IDE's and bug finders to perform source code analysis and indexing.
...
\myProperName{Clang} works on all popular operating systems, including Microsoft Windows. Having this advantage over Dehydra, \myProperName{Clang} is the favourite choice for use in this project. A detailed description of how \myProperName{libclang} is used inside the application can be found in \myRefChapter{chap:AnalysingCPP}.

\subsubsection{\myProperName{Emscripten}}

In the context of this thesis, another interesting \myProperName{LLVM}-based project should be mentioned. \myInlineQuote{Emscripten is an LLVM-to-JavaScript compiler. It takes LLVM bitcode (which can be generated from C/C++ using Clang, or any other language that can be converted into LLVM bitcode) and compiles that into JavaScript, which can be run on the web.}\footnote{\citep{EmscriptenHP} - taken from front page} Instead of accessing C++ through glue code, the C++ code itself is completely compiled to \myProperName{JavaScript}. This may sound very slow in the first place, but with the current performance of \myProperName{JavaScript} virtual machines and the reduced cost of type-conversion (as all is \myProperName{JavaScript}), code generated by \myProperName{Emscripten} is only 3 to 4 times slower than native code generated with \myProperName{GCC}.\footnote{\citep{EmscriptenHP} - taken from FAQ section}