...
The creation of the simplified AST happens inside the \mySCName{Clang\_AST} class. Different data structures have been written to hold information about AST nodes and types. All AST nodes inherit from the class \mySCName{ASTObject}. The simplified AST will only contain information about the following AST objects: global variable declarations, unions, typedefs, structs, (templated) classes, (templated) functions, function parameters, member functions, constructors, destructors, fields (data members), enums, enum constants, namespaces, template parameters and template arguments.
...
\todo{comparison: source-code, CXCursor-AST, ASTObject AST}
