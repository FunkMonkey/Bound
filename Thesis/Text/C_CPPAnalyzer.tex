...
To retrieve information about elements declared in C++ header files a shared library called \myProperNameImp{CPPAnalyzer} was written in C++. The library uses \myProperNameImp{libclang} for parsing C++ files and accessing AST information. Internally a reduced syntax tree is built and exported as \myProperName{JSON} (JavaScript Object Notation).\todo{explain json}
...
As explained in \myRefSection{sec:Clang}, \myProperName{libclang} is a C interface to \myProperName{Clang} that can be used for accessing \myProperName{Clang}'s parsing functionality and internal AST.
...
\\\mySCNameImp{CXCursor} represents a single node in the AST. It holds an enum constant (named kind) that identifies the kind of the AST node - whether it is a namespace, function declaration, class declaration, etc. It also saves information about the according \myProperName{Clang} data in an array of void pointers.
\\\mySCNameImp{CXType} represents a C++ type. It holds the kind of the type, for example if it is a pointer, reference, builtin type (like int or float), record (class or struct), etc. Just like \mySCName{CXCursor}, it also saves information about the \myProperName{Clang} type representation.
...
During this thesis it became clear that \myProperName{libclang} needed to be extended by the author to expose AST information about C++ templates. It seems, such information was not needed by other \myProperName{libclang} users. The changes made to the \myProperName{libclang} source code base will hopefully make its way back into the official \myProperName{Clang} repository.
...
The \myProperName{CPPAnalyzer} application provides a C interface through which it can be called.

\SingleSpacing
\begin{lstlisting}[language=C++, caption=\myProperName{CPPAnalyzer} C API]
struct ParserInfo
{
	char* astTreeJSON;
};
...
\section{\myProperName{libclang} and the C++ standard library}

During the development, problems occurred concerning the C++ standard library.
